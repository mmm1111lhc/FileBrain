...
| 🏷️ **智能命名** | 提取文件内容 → 自动生成 `内容摘要_日期_版本号` 文件名 |
...
| 场景 | 效果 |
|------|------|
| 🏛️ **政务软件下载**（浙里办、随申办等） | 申请表/回执单 → 自动识别标题并重命名 |
| 💼 **办公文档管理** | 合同/方案/报告 → 按内容摘要+版本号自动归档 |
| 📚 **学术资料整理** | 论文/扫描件 → OCR 识别 + 全文检索 |
| 🏠 **个人文件归档** | 身份证/证件扫描件 → 自动识别并命名 |

> **比如从浙里办下载审批回执：** 原始文件名是 `20260622_12345678.pdf`，FileBrain 自动识别内容后重命名为 `建设工程规划许可证_20260622_v1.0.pdf`，再也不用一个一个手动改文件名。
