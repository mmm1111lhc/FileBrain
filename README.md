...
| 🏷️ **智能命名** | 提取文件内容 → 自动生成 `内容摘要_日期[_作者]` 文件名 |
...
### 🔹 第一步：双击 `setup.command`（安装）

> 📁 在下载的 `FileBrain-main` 文件夹里，找到 **`setup.command`**
> 双击它 → 系统可能会提示"无法验证开发者"
...
> 安装完成后，**桌面上会出现一个 `FileBrain启动器.command`**
> 双击它 → 工具就启动了！✨

> ⚠️ 如果启动器没出现，手动进文件夹双击 `start.command` 也行
